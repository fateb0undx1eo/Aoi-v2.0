...
const AutoResponder = require('../src/schemas/autoResponderSchema');
const { v4: uuidv4 } = require('uuid');

// Get all autoresponder triggers for a guild
app.get('/api/autoresponder', async (req, res) => {
    try {
        const firstGuild = client.guilds.cache.first();
        if (!firstGuild) {
            return res.status(404).json({ success: false, message: 'No guilds found' });
        }

        let autoConfig = await AutoResponder.findOne({ guildId: firstGuild.id });
        
        if (!autoConfig) {
            autoConfig = await AutoResponder.create({
        ...
        res.json({ 
            success: true, 
            triggers: autoConfig.triggers || [],
            guildId: firstGuild.id,
            guildName: firstGuild.name
        });
    } catch (error) {
        console.error('[API] Error fetching autoresponder triggers:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

// Add new autoresponder trigger
app.post('/api/autoresponder', async (req, res) => {
    try {
        ...
        const { type, pattern, response, caseSensitive, deleteOriginal, cooldown, embed } = req.body;

        if (!type || !pattern || !response) {
            return res.status(400).json({ 
                success: false, 
                message: 'Type, pattern, and response are required' 
            });
        }
        ...
        autoConfig.triggers.push(newTrigger);
        await autoConfig.save();

        console.log(`[AutoResponder] ✓ New trigger added: "${pattern}" (${type})`);
        ...
        console.error('[API] Error adding autoresponder trigger:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

// Update autoresponder trigger
app.put('/api/autoresponder/:id', async (req, res) => {
    try {
        const firstGuild = client.guilds.cache.first();
        if (!firstGuild) {
            return res.status(404).json({ success: false, message: 'No guilds found' });
        ...
        const autoConfig = await AutoResponder.findOne({ guildId: firstGuild.id });
        
        if (!autoConfig) {
            return res.status(404).json({ success: false, message: 'No autoresponder config found' });
        }

        const triggerIndex = autoConfig.triggers.findIndex(t => t.id === req.params.id);
        ...
        Object.assign(autoConfig.triggers[triggerIndex], req.body);
        await autoConfig.save();

        console.log(`[AutoResponder] ✓ Trigger updated: ${req.params.id}`);

        res.json({ 
            success: true, 
            message: 'Trigger updated successfully',
            trigger: autoConfig.triggers[triggerIndex]
        });
    } catch (error) {
        console.error('[API] Error updating autoresponder trigger:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

// Toggle autoresponder trigger
app.patch('/api/autoresponder/:id/toggle', async (req, res) => {
    try {
        const firstGuild = client.guilds.cache.first();
        if (!firstGuild) {
            return res.status(404).json({ success: false, message: 'No guilds found' });
        ...
        const trigger = autoConfig.triggers.find(t => t.id === req.params.id);
        ...
        console.log(`[AutoResponder] ✓ Trigger ${trigger.enabled ? 'enabled' : 'disabled'}: ${req.params.id}`);
        ...
        console.error('[API] Error toggling autoresponder trigger:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});

// Delete autoresponder trigger
app.delete('/api/autoresponder/:id', async (req, res) => {
    try {
        const firstGuild = client.guilds.cache.first();
        if (!firstGuild) {
            return res.status(404).json({ success: false, message: 'No guilds found' });
        ...
        autoConfig.triggers.splice(triggerIndex, 1);
        await autoConfig.save();

        console.log(`[AutoResponder] ✓ Trigger deleted: ${req.params.id}`);
        ...
        console.error('[API] Error deleting autoresponder trigger:', error);
        res.status(500).json({ success: false, message: error.message });
    }
});
