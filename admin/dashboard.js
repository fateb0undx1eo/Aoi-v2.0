...
app.get('/api/bot-config', async (req, res) => {
    try {
        const botConfigService = client.botConfigService;
        if (!botConfigService) {
            return res.status(500).json({ error: 'Bot configuration service not available' });
        }
        ...
        res.json({
            ...
            config: {
                ...
                appearance: {
                    ...
                    bannerUrl: config.appearance.bannerUrl
                },
...
// Update bot presence
app.post('/api/bot-config/presence', async (req, res) => {
    try {
        const botConfigService = client.botConfigService;
        if (!botConfigService) {
            return res.status(500).json({ error: 'Bot configuration service not available' });
        ...
        const { status, activities } = req.body;
        
        if (!status || !activities) {
            return res.status(400).json({ error: 'Status and activities are required' });
        }

        // Get user ID from token (simplified - assumes token contains user info)
        const token = req.headers.authorization?.replace('Bearer ', '');
        const decoded = Buffer.from(token, 'base64').toString('utf-8');
        const [username] = decoded.split(':');
        
        // For dashboard, we use the bot owner ID from env
        const userId = process.env.BOT_OWNER_ID;
        ...
        const result = await botConfigService.updatePresence(presenceConfig, userId, 'dashboard');
...
// Update bot avatar
app.post('/api/bot-config/avatar', async (req, res) => {
    try {
        const botConfigService = client.botConfigService;
        if (!botConfigService) {
            return res.status(500).json({ error: 'Bot configuration service not available' });
        ...
        const userId = process.env.BOT_OWNER_ID;
        const result = await botConfigService.updateAvatar(imageUrl, userId, 'dashboard');
...
// Update bot username
app.post('/api/bot-config/username', async (req, res) => {
    try {
        const botConfigService = client.botConfigService;
        if (!botConfigService) {
            return res.status(500).json({ error: 'Bot configuration service not available' });
        ...
        const userId = process.env.BOT_OWNER_ID;
        const result = await botConfigService.updateUsername(newName, userId, 'dashboard');
...
// Update bot banner
app.post('/api/bot-config/banner', async (req, res) => {
    try {
        const botConfigService = client.botConfigService;
        if (!botConfigService) {
            return res.status(500).json({ error: 'Bot configuration service not available' });
        ...
        const userId = process.env.BOT_OWNER_ID;
        const result = await botConfigService.updateBanner(imageUrl, userId, 'dashboard');
...
app.get('/api/bot-config/rate-limits', async (req, res) => {
    try {
        const botConfigService = client.botConfigService;
        if (!botConfigService) {
            return res.status(500).json({ error: 'Bot configuration service not available' });
